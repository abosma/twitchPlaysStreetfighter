...
import asyncio
import twitchPlaysStreetfighterp2.movesController as mc
...
class IRCController():
    ...
    def send_message(self, message):
        self.s.send(bytes("PRIVMSG #atillabosmathesecond :" + message + "\r\n", "UTF-8"))
   
    async def bot(self):
        self.s.connect((self.HOST, self.PORT));
        self.s.send(bytes("PASS " + self.PASS + "\r\n", "UTF-8"))
        self.s.send(bytes("NICK " + self.NICK + "\r\n", "UTF-8"))
        self.s.send(bytes("JOIN #atillabosmathesecond \r\n", "UTF-8"))
        while True:
